...
function initializeRoadmapChart() {
    ...
    for (let year = 1; year <= 20; year++) {
        const yearData = roadmapData.map(phase => (year >= phase.startYear && year <= phase.endYear) ? 1 : 0);
        chartDatasets.push({
            label: `Year ${year}`,
            data: yearData,
            borderColor: '#1a472a',
    ...
    const roadmapChart = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                x: { stacked: true, max: 20, ticks: { callback: v => v }, title: { display: true, text: 'Project Years', font: { size: 12, weight: 'bold' } } },
                y: { stacked: true, title: { display: true, text: 'Project Phases', font: { size: 12, weight: 'bold' } } }
            }
        }
    });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Commented out to prevent overwriting the hero canvas
